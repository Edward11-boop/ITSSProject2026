...
const T2Etaj2 = ({ onRoomSelect, onSeatSelect, onOccupiedSelect }: MapProps) => {
  const { handleSeatClick, getSelectedState } = useSeatSelection([
    { groupId: 'G-O2', matches: (id: string) => id.includes('T2-O2') }
  ]);
  ...
  const handleSeatSelection = (id: string, type?: 'individual' | 'room') => {
    ...
    const occupiedSeats = [
      "T2-SD2-01", "T2-SD2-02", "T2-B2-06",
      "T2-B2-07", "T2-B2-13", "T2-B2-14"
    ];
