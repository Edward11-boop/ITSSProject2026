...
const InviteModal = () => {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!response.ok) {
      ...
      console.error("Invitation was not saved", response.status, errorText);
  ...
  return (
    <>
      {submitError !== "" && (
        <ErrorPopUp
          title="Invitatia nu a fost trimisa"
          message={submitError}
      ...
      <div className="w-full max-w-xl rounded-[32px] border border-[#C4B5FD] bg-[#EDE9FE] px-5 py-6 shadow-xl sm:rounded-[48px] sm:px-10 sm:py-10">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button
            ...
            className={`mt-8 w-full rounded-full px-6 py-4 text-lg font-bold text-white sm:mt-12 sm:text-xl ${isFormInvalid
